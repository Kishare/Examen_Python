...
class Task:
	...
	def need_to_run(self):
		...
		if datetime.datetime.now() > self.NEXT_DEADLINE + datetime.timedelta(seconds=self.PERIOD):
			...
			print("\tTask " + self.NAME + "\ŧFAILED deadline.\t" + str(self.EXECUTION_DONE/self.EXECUTION_TIME) + "% done.")
	...
	def run(self):
		...
		if self.IS_INTERRUPTIBLE == False :
			...
			if self.NEXT_DEADLINE > datetime.datetime.now().minute:
				print(str(timer) + "\tTask " + self.NAME + " finished his job : " + str(self.EXECUTION_DONE/self.EXECUTION_TIME) + "%")
				self.EXECUTION_DONE = 0
				self.NEXT_DEADLINE = self.NEXT_DEADLINE + datetime.timedelta(seconds=self.PERIOD)
				return
